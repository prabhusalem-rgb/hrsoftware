-- ============================================================
-- Migration 024: Comprehensive Payout System Tables
-- Creates payout_runs, payout_items (batch), approvals,
-- notifications, bank reconciliation, and supporting objects
-- ============================================================
...
-- ============================================================
-- 2. PAYOUT ITEMS (Batch Level) - Individual employee payout
...
CREATE TABLE IF NOT EXISTS payout_items (
  ...
  -- Payout status (mirrors/synced with payroll_items.payout_status)
...
-- Update updated_at trigger function (reuse if exists)
...
-- ============================================================
-- 11. HELPER FUNCTION: create_payout_run_from_payroll
-- ============================================================
...
-- ============================================================
-- 12. HELPER FUNCTION: get_payout_summary
-- ============================================================

CREATE OR REPLACE FUNCTION get_payout_summary(p_payout_run_id UUID)
RETURNS JSONB AS $$
DECLARE
  v_summary JSONB;
BEGIN
  SELECT jsonb_build_object(
    'total_items', COUNT(*),
    'pending_count', COUNT(*) FILTER (WHERE payout_status = 'pending'),
    'held_count', COUNT(*) FILTER (WHERE payout_status = 'held'),
    'processing_count', COUNT(*) FILTER (WHERE payout_status = 'processing'),
    'paid_count', COUNT(*) FILTER (WHERE payout_status = 'paid'),
    'failed_count', COUNT(*) FILTER (WHERE payout_status = 'failed'),
    'pending_amount', COALESCE(SUM(CASE WHEN payout_status = 'pending' THEN paid_amount ELSE 0 END), 0),
    'held_amount', COALESCE(SUM(CASE WHEN payout_status = 'held' THEN paid_amount ELSE 0 END), 0),
    'processing_amount', COALESCE(SUM(CASE WHEN payout_status = 'processing' THEN paid_amount ELSE 0 END), 0),
    'paid_amount', COALESCE(SUM(CASE WHEN payout_status = 'paid' THEN paid_amount ELSE 0 END), 0),
    'failed_amount', COALESCE(SUM(CASE WHEN payout_status = 'failed' THEN paid_amount ELSE 0 END), 0)
  ) INTO v_summary
  FROM payout_items
...
-- Super admin can access all payout data
CREATE POLICY "Super admin full access to payout_runs" ON payout_runs
  FOR ALL USING (
    EXISTS (
...
CREATE POLICY "Super admin full access to payout_items" ON payout_items
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid()
...
-- Company admin and finance can access their company's payout data
CREATE POLICY "Company users access payout_runs" ON payout_runs
  FOR ALL USING (
    company_id IN (
...
CREATE POLICY "Company users access payout_items" ON payout_items
  FOR ALL USING (
    payout_run_id IN (
...
-- HR and Viewer can read payout data (for reports/viewing)
CREATE POLICY "HR can view payout_runs" ON payout_runs
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid()
      AND (role = 'hr' OR role = 'viewer')
      AND profiles.company_id = payout_runs.company_id
    )
  );

CREATE POLICY "HR can view payout_items" ON payout_items
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM payout_runs pr
      JOIN profiles p ON p.company_id = pr.company_id
      WHERE pr.id = payout_items.payout_run_id
        AND p.id = auth.uid()
        AND (p.role = 'hr' OR p.role = 'viewer')
    )
  );

-- ============================================================
-- 14. AUDIT VIEW - Comprehensive payout audit trail
-- ============================================================
...
LEFT JOIN audit_logs al ON al.entity_type = 'payout_item' AND al.entity_id = pi.id
LEFT JOIN profiles p ON al.user_id = p.id
ORDER BY al.created_at DESC;

-- ============================================================
-- 15. PAYOUT SUMMARY VIEW - Real-time aggregated stats
-- ============================================================
...
SELECT
  ...
  COALESCE(SUM(CASE WHEN pi.payout_status = 'held' THEN pi.paid_amount ELSE 0 END), 0) as held_amount,
...
-- 2. Run the TypeScript type update (task 3)
-- 3. Create the payout management pages (task 4, 5)
-- ============================================================
