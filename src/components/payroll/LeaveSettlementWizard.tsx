...
export function LeaveSettlementWizard({ isOpen, onClose, employees, onProcess, preselectedEmployeeId, preselectedLeaveId }: LeaveSettlementWizardProps) {
  ...
  // (balance already deducts this leave, so we need original available = entitled + carried_forward - lapsed)
  const originalEntitlement = (annualBalanceRecord?.entitled || 0) + (annualBalanceRecord?.carried_forward || 0) - (annualBalanceRecord?.lapsed || 0);
