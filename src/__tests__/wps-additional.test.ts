...
describe('Vacation employee handling in WPS', () => {
  it('should add 1.000 OMR for on_leave employee not in payroll and no settlement/rejoin', () => {
    const company: Company = {
      id: 'comp-1',
      name: 'Test Company',
      cr_number: '123456',
    ...
    expect(exportedAmounts.get(vacationExportId)).toBe(1.000);
    expect(exportedAmounts.get(payrollItem.id)).toBe(1350);
    expect(sifContent).toContain('VAC01');
    expect(sifContent).toContain('VACATION');
    expect(sifContent).toContain('1.000');
  });

  it('should NOT add 1.000 if leave_settlement_date is in same month', () => {
    const company: Company = {
      id: 'comp-1',
      name: 'Test Company',
      cr_number: '123456',
  ...
  it('should NOT add 1.000 if rejoin_date is in same month', () => {
    const company: Company = {
      id: 'comp-1',
      name: 'Test Company',
      cr_number: '123456',
  ...
  it('should add 1.000 OMR for leave_settled employee (settlement done before vacation)', () => {
    // Scenario: Leave settlement processed on May 10, employee on leave May 15 - July 13
    // June WPS export should include 1.000 OMR (settlement not in June, rejoin not in June)
    const company: Company = {
      id: 'comp-1',
      name: 'Test Company',
      cr_number: '123456',
    ...
    // Vacation employee should have 1.000 OMR entry
    const vacationExportId = `vacation-${vacationEmp.id}`;
    expect(exportedAmounts.get(vacationExportId)).toBe(1.000);
  ...
  it('should NOT add 1.000 for leave_settled employee if leave_settlement_date is in same month', () => {
  ...
  it('should NOT add 1.000 for leave_settled employee if rejoin_date is in same month', () => {
  ...
  it('should include vacation 1.000 in total amount calculation', () => {
    const company: Company = {
      id: 'comp-1',
      name: 'Test Company',
      cr_number: '123456',
    ...
    expect(totalSalaries).toBe('1.000');
  });
  ...
  describe('exportMode options', () => {
    ...
    it('should generate vacation_only (excluding regular salaries)', () => {
      ...
      expect(exportedAmounts.get(`vacation-${vacationEmp.id}`)).toBe(1.000);
      expect(exportedAmounts.size).toBe(1);
    });
  });
});
