...
describe('Timesheet API', () => {
  ...
  describe('POST /api/timesheet', () => {
    ...
    it('should create holiday_overtime timesheet successfully with valid hours (1-8)', async () => {
      const mockClient = new SupabaseMockClient();
      mockClient.auth.getSession = vi.fn().mockResolvedValue({
        data: { session: { user: { id: 'user-123' } } },
      });
      ...
      expect(response.status).toBe(201);
      expect(data.data.day_type).toBe('holiday_overtime');
      expect(data.data.hours_worked).toBe(0);
      expect(data.data.overtime_hours).toBe(5);
    });

    it('should reject holiday_overtime with hours_worked > 0', async () => {
      const mockClient = new SupabaseMockClient();
      mockClient.auth.getSession = vi.fn().mockResolvedValue({
        data: { session: { user: { id: 'user-123' } } },
      });
      ...
      const req = createMockRequest({
        ...
        date: '2025-05-15',
        day_type: 'holiday_overtime',
        hours_worked: 4, // Invalid: should be 0
        overtime_hours: 4,
        company_id: 'comp-1',
        reason: 'Holiday work',
      });
    ...
    it('should reject holiday_overtime with overtime_hours = 0', async () => {
      const mockClient = new SupabaseMockClient();
      mockClient.auth.getSession = vi.fn().mockResolvedValue({
        data: { session: { user: { id: 'user-123' } } },
      });
      ...
      const req = createMockRequest({
        ...
        day_type: 'holiday_overtime',
        hours_worked: 0,
        overtime_hours: 0, // Invalid: must be >= 1
        company_id: 'comp-1',
        reason: 'Holiday work',
      });
    ...
    it('should reject holiday_overtime with overtime_hours > 8', async () => {
      const mockClient = new SupabaseMockClient();
      mockClient.auth.getSession = vi.fn().mockResolvedValue({
        data: { session: { user: { id: 'user-123' } } },
      });
      ...
      const req = createMockRequest({
        ...
        day_type: 'holiday_overtime',
        hours_worked: 0,
        overtime_hours: 9, // Invalid: max is 8
        company_id: 'comp-1',
        reason: 'Holiday work',
      });
    ...
    it('should reject holiday_overtime without reason (overtime > 0)', async () => {
      const mockClient = new SupabaseMockClient();
      mockClient.auth.getSession = vi.fn().mockResolvedValue({
        data: { session: { user: { id: 'user-123' } } },
      });
      ...
      const req = createMockRequest({
        ...
        overtime_hours: 5,
        company_id: 'comp-1',
        reason: '', // Empty reason required for OT
      });
