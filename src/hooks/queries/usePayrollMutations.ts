  ...
  const deletePayrollRun = useMutation({
    mutationFn: async (runId: string) => {
      ...
      if (run.type === 'leave_settlement' && items.length > 0) {
        ...
        const settledEmployeeIds = new Set(items.map((i: any) => i.employee_id));
        ...
        console.log('[processPayroll] settledEmployeeIds:', Array.from(settledEmployeeIds).map((id: any) => id.substring(0,8)));
