...
export function usePayrollMutations(companyId: string) {
  ...
  const processPayroll = useMutation({
    mutationFn: async ({ run, items }: { run: any; items: PayrollItem[] }) => {
      ...
      // If Vacation Payroll: Create a 1 OMR Vacation Advance Loan for each employee
      const isVacationRun = run.notes && run.notes.includes('Vacation Payroll');
      if (isVacationRun) {
        const vacationYear = run.year;
        const vacationMonth = run.month;
        const disbursementDate = `${vacationYear}-${String(vacationMonth).padStart(2, '0')}-01`;
        // Tentative due date: following month
        const nextMonth = vacationMonth === 12 ? 1 : vacationMonth + 1;
        const nextYear = vacationMonth === 12 ? vacationYear + 1 : vacationYear;
        const defaultDueDate = `${nextYear}-${String(nextMonth).padStart(2, '0')}-01`;

        for (const item of itemsWithRunId) {
          try {
            // Check if vacation loan already created for this employee for this month
            const { data: existingLoan } = await supabase
              .from('loans')
              .select('id')
              .eq('employee_id', item.employee_id)
              .eq('principal_amount', 1)
              .ilike('notes', `%Vacation Payroll%${vacationMonth}/${vacationYear}%`)
              .limit(1);
            ...
            if (!existingLoan || existingLoan.length === 0) {
              const { data: newLoan, error: loanErr } = await supabase
                ...
                .insert({
                  ...
                  company_id: run.company_id,
                  principal_amount: 1.000,
                  interest_rate: 0,
                  tenure_months: 1,
                  disbursement_date: disbursementDate,
                  first_payment_date: defaultDueDate,
                  monthly_emi: 1.000,
                  total_interest: 0,
                  total_amount: 1.000,
                  balance_remaining: 1.000,
                  status: 'active',
                  notes: `Vacation Payroll Advance (${vacationMonth}/${vacationYear})`,
                })
                .select()
                .single();
            ...
            console.error('[Vacation Loan Creation] Failed for employee:', item.employee_id, loanCreateErr);
          }
        }
      }
    ...
    mutationFn: async (runId: string) => {
      ...
      if (run.type === 'leave_settlement' && items.length > 0) {
        ...
        for (const empId of employeeIds) {
          ...
          const nonSickLeaves = (activeLeaves || []).filter((l: any) => {
            const typeName = l.leave_types?.name || '';
            return !typeName.toLowerCase().includes('sick');
          });

          const newStatus = nonSickLeaves.length > 0 ? 'on_leave' : 'active';
          const { error: empError } = await supabase
            .from('employees')
