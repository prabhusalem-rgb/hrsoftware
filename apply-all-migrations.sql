-- ============================================================
-- Combined Migration: 106 (Leave Requests Module) + 108 RLS GRANTs
-- Idempotent — safe to run multiple times
-- ============================================================

-- ========== Migration 106: Leave Requests Module ==========
...
-- 4. Storage Bucket for Leave Signatures
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'set_timestamp_leave_requests'
  ) THEN
...
-- ========== Migration 108: Public Leave Request RLS (GRANTs) ==========

-- Grant SELECT on tables to anon and authenticated roles (required for RLS to work)
GRANT SELECT ON companies TO anon, authenticated;
GRANT SELECT ON employees TO anon, authenticated;
GRANT SELECT ON leave_requests TO anon, authenticated;

-- RLS policies for anon access (already applied above if running combined, but safe to re-run)
...
-- Also ensure authenticated users can access these tables
