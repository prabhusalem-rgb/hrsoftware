-- ============================================================
-- Auto-create leave management record when leave request becomes GM Approved
...
-- 1. Ensure 'approved' is allowed in leave_requests status
ALTER TABLE leave_requests DROP CONSTRAINT IF EXISTS leave_requests_status_check;
ALTER TABLE leave_requests ADD CONSTRAINT leave_requests_status_check
  CHECK (status IN ('pending', 'hr_approved', 'gm_approved', 'approved', 'rejected'));

-- 2. Add column to link leave management record back to original leave request
...
-- 4. Drop ALL possible trigger variants (clean up any old versions)
...
-- 5. Create the trigger function (correct: inserts into leaves, NOT leave_requests)
...
-- 6. Create the trigger (AFTER UPDATE, row-level)
